...
export async function PATCH(request, { params }) {
  ...
  const campos = {};
  for (const clave of ["nombre", "icono", "color", "orden"]) {
    if (body[clave] !== undefined) campos[clave] = body[clave];
  }

  const { data, error } = await supabaseAdmin.from("mb_categorias").update(campos).eq("id", id).select().single();
...
export async function DELETE(request, { params }) {
  ...
  const { id } = await params;
  const { error } = await supabaseAdmin.from("mb_categorias").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
