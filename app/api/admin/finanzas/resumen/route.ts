...
async function calcularPaso(soloHoy: boolean): Promise<PasoFlujoCaja> {
  ...
  let queryMovs = supabaseAdmin.from("movimientos").select("tipo, efectivo, transferencia, monto, created_at");
  ...
  let queryCxc = supabaseAdmin.from("cuentas_por_cobrar").select("monto, fecha_cobro").eq("estado", "cobrado");
  ...
  let queryProvision = supabaseAdmin.from("mayor_provision").select("tipo, monto, fecha");
...
export async function GET() {
  ...
  const [diario, acumulado] = await Promise.all([calcularPaso(true), calcularPaso(false)]);
